...
function PurchaseTransfer({ value, grave, handleClose }) {
  // קבלת ערכים מהסטור
  ...
  const { getEntityByAttr, getEntitiesByAttr, addOrUpdateDataPurchase, AllDataSignatures, addOrUpdateDataSignature, duplicateAndUpdatePurchase } = useQueries();
  ...
  const myColumns = (localColumns = null) => {
    ...
    return localColumns.map((field, index) => {
      let lock = false
      lock = (
        field.name !== 'isFuneralPayments' && field.name !== 'BookkeepingApproval' && field.name !== 'clientId'
      ) ? true : false
      return ({
        ...field,
  ...
  const handleFormSubmit = (submittedSignatures) => {
    ...
    async function executeQueries(newFormValues) {
      try {
        ...
        // שאילתה ראשונה
        let resDuplicateAndUpdatePurchase = duplicateAndUpdatePurchase(newFormValues);

        // // שאילתה רביעית - חתימות
        // let signaturesRequests = submittedSignatures.map(element => {
        //   element = { ...element };
        //   return addOrUpdateDataSignature(element);
        // });

        // מחכה לכל השאילתות ביחד
        let results = await Promise.all([
          resDuplicateAndUpdatePurchase
