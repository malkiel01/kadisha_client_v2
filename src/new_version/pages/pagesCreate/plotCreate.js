...
function PlotCreate() {
 ...
 const handleAllDataLoaded = () => {
   ...
   if (value?.id) {
    let data = getEntityByAttr(localBlocks, 'id', value?.id);
      ...
      localBlocks.map(block => {
        return CommponentOptions({
          value: block?.id, label: block?.blockNameHe, dataUnderRigth: {
            title: 'בית עלמין: ',
            value: getEntityByAttr(localCemeteries, 'id', block?.cemeteryId)?.cemeteryNameHe
          }
        })
      })
    )
  ...
  const fieldAuto = (data, newFormValues) => {
    ...
    const block = getEntityByAttr(localBlocks, 'id', data?.id)
