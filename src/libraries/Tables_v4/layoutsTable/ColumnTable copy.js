...
const ColumnTable = ({ column }) => {
  ...
  const handleMouseDown = (e) => {
    e.preventDefault();
    const cellRect = e.target.closest('td').getBoundingClientRect();
    const spanRect = e.target.getBoundingClientRect();
    console.log('Cell width:', cellRect.width);
    console.log('Span position relative to cell:', spanRect.left - cellRect.left);
    console.log('Mouse position on screen:', e.clientX, e.clientY);
  };
  ...
  return <TableCell ref={tableCellRef} style={{ ...theme.titleTable, minWidth: width, maxWidth: width, position: 'relative' }}
  >
