...
import AreaGraveDiv from './AreaGraveDiv'


const AreaGraveCard = ({ plot }) => {
    ...
    <Card sx={{ minWidth: 275, marginTop: 2, marginBottom: 2 }}>
      <CardContent>
        <Typography variant="h5" component="div">
           {plot?.plotNameHe} ({plot?.plotNameEn})
        </Typography>
        <Typography sx={{ mb: 1.5 }} color="text.secondary">
          קוד ביטוח לאומי: {plot?.nationalInsuranceCode || '----'}
        </Typography>
        <Typography variant="body2">
          <Grid container>
            ...
            <Grid item xs={4}>
            קואורדינטות: {plot?.coordinates || '----'}
            </Grid>
          </Grid>
          <Grid container>
            <Grid item xs={4}>
            תאריך יצירה: {plot?.createDate || '----'}
            </Grid>
            <Grid item xs={4}>
            תאריך עדכון אחרון: {plot?.inactiveDate || '----'}
            </Grid>
          </Grid>
          <Grid container>
            <Grid item xs={4}>
            איש קשר: {plot?.contactName || '----'} טלפון: {plot?.contactPhoneName || '----'}
            </Grid>
            <Grid item xs={4}>
            פעיל: {plot?.isActive ? 'כן' : 'לא'}
            </Grid>
          </Grid>
        </Typography>
          ...
          <Typography variant="body2">
          <Grid container>
            <AreaGraveDiv exemption = {items} unusual = {items} closed = {items}/>
            </Grid>
          </Typography>
      </CardContent>
    </Card>
